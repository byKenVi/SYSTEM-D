...
export default function PortalProductDetail({ viewAsContactId }: { viewAsContactId?: number }) {
  ...
  const purchaseMutation = useMutation({
    ...
    onSuccess: (order: any) => {
      ...
      toast({ title: "Commande confirmée", description: "Le débit Store Credit et la commande Shopify ont été confirmés." });
      ...
      <Dialog open={purchaseOpen} onOpenChange={setPurchaseOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Votre Store Credit sera débité, puis une vraie commande payée sera créée dans Shopify. Aucun Bon de Travail ne sera créé.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => purchaseMutation.mutate()} disabled={purchaseMutation.isPending || Number(purchaseQty) < 1 || Number(purchaseQty) > product.inventoryQuantity || !currentRep || requestedTotal > Number(currentRep.balance)} data-testid="button-confirm-purchase">
              {purchaseMutation.isPending ? "Confirmation Shopify…" : "Confirmer avec Store Credit"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
